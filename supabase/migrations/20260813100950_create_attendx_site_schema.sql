...
1. `site_config` — single-row config table (id fixed = 1).
   - app_name, tagline, version
   - hero_headline, hero_description, hero_image_url
   - apk_url, apk_version, apk_release_date, apk_size, apk_release_notes
...
- Public read access (anon + authenticated) on all content tables so the public
  site can render without a login.
- Write access (INSERT/UPDATE/DELETE) restricted to `authenticated` only.
  Only the admin has a Supabase account, so only the admin can edit content.
- No user_id columns: content is shared site-wide, not per-user.
*/
