  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">{items.length} features</p>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="grid flex-1 gap-3 sm:grid-cols-[1fr_1fr_140px]">
                <input
                  ...
                  onChange={(e) =>
                    setItems((arr) =>
                      arr.map((x) => (x.id === f.id ? { ...x, title: e.target.value } : x))
                    )
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setItems((arr) =>
                      arr.map((x) => (x.id === f.id ? { ...x, description: e.target.value } : x))
                    )
                  }
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                ...
                <input
                  ...
                  onChange={(e) =>
                    setItems((arr) =>
                      arr.map((x) => (x.id === f.id ? { ...x, icon: e.target.value } : x))
                    )
                  }
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
